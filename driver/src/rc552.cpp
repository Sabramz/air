...
// 	// controlBuffer.
// 	uint8_t controlBuffer[2];
// 	int status = calculateCRC(&backData[0], *backLen - 2,
// &controlBuffer[0]); 	if (status != 0) { 		return status;
// 	}
// 	if ((backData[*backLen - 2] != controlBuffer[0]) ||
// 		(backData[*backLen - 1] != controlBuffer[1])) {
// 		return 10;
...
//  * Only "new" cards in state IDLE are invited. Sleeping cards in state
//  HALT are
//  * ignored.
//  *
//  * @return bool
...
// 	uint8_t
// 		*bufferATQA,    ///< The buffer to store the ATQA (Answer to
// request) in 	uint8_t *bufferSize ///< Buffer size, at least two bytes.
// Also number of
...
// 	uint8_t command, ///< The command to send - PICC_CMD_REQA or
// PICC_CMD_WUPA 	uint8_t 		*bufferATQA,    ///< The buffer to store
// the ATQA (Answer to request) in 	uint8_t *bufferSize ///< Buffer size, at
// least two bytes. Also number of
// 						///< bytes returned if STATUS_OK.
// ) {
// 	uint8_t validBits;
// 	int status;
...
// 	validBits =
// 		7; // For REQA and WUPA we need the short frame format - transmit
// only 7
// 		   // bits of the last (and only) byte. TxLastBits =
// BitFramingReg[2..0] 	status = transcieveData(&command, 1, bufferATQA,
// bufferSize, &validBits); 	if (status != STATUS_OK) { 		return
// status;
// 	}
// 	if (*bufferSize != 2 || validBits != 0) { // ATQA must be exactly 16
// bits. 		return STATUS_ERROR;
// 	}
// 	return STATUS_OK;
...
//  * Remember to call this function after communicating with an
//  authenticated PICC
...
// ) {
// 	uint8_t firstBlock; // Address of lowest address to dump actually last
// block
// 						// dumped)
// 	uint8_t no_of_blocks; // Number of blocks in sector
// 	bool isSectorTrailer; // Set to true while handling the "last" (ie
// highest
...
// 	} else { // Illegal input, no MIFARE Classic PICC has more than 40
// sectors. 		return;
// 	}
...
// 	for (int blockOffset = no_of_blocks - 1; blockOffset >= 0;
// blockOffset--) { 		blockAddr = firstBlock + blockOffset;

// 		MIFARE_dump_sector(blockAddr, isSectorTrailer, firstBlock, key,
// buffer, 			no_of_blocks, blockOffset);
// 	}
...
// 						 (acc2 != (~acc2Inv & 0xF)) ||
// 						 (acc3 != (~acc3Inv & 0xF));
// 		group[0] = ((acc1 & 1) << 2) | ((acc2 & 1) << 1) | ((acc3Inv & 1) <<
// 0); 		group[1] = ((acc1 & 2) << 1) | ((acc2 & 2) << 0) | ((acc3Inv &
// 2) >> 1); 		group[2] = ((acc1 & 4) << 0) | ((acc2 & 4) >> 1) |
// ((acc3Inv & 4) >> 2); 		group[3] = ((acc1 & 8) >> 1) | ((acc2 & 8)
// >> 2) | ((acc3Inv & 8) >> 3); 		*isSectorTrailer = false;
// 	}
// }
...
// 	// 32-39) 		g[0]	Access bits for block 0 (for sectors 0-31) or
// blocks
// 	// 0-4 (for sectors 32-39)
// 	// Each group has access bits [C1 C2 C3]. In this code C1 is MSB and C3
// is
// 	// LSB. The four CX bits are stored together in a nible cx and an
// inverted
// 	// nible cx_.
...
// 	if (isSectorTrailer) {
// 		int status = PCD_Authenticate(PICC_CMD_MF_AUTH_KEY_A, firstBlock,
// key); 		if (status != STATUS_OK) { printf("PCD_Authenticate()
// failed: "); 			printf("%d", status); 			return;
// 		}
// 	}
...
// 						(int32_t(buffer[2]) << 16) | (int32_t(buffer[1]) <<
// 8) | 						int32_t(buffer[0]); 		printf("
// Value=0x"); 		printf("%X", value); 		printf(" Adr=0x");
// printf("%X", buffer[12]);
// 	}
// 	printf("\n");
// }
...
//  communication
//  * to any MIFARE Mini, MIFARE 1K and MIFARE 4K card. The authentication
//  is
...
//  ACTIVE(*)
//  * - before calling this function. Remember to call PCD_StopCrypto1()
//  after
//  * communicating with the authenticated PICC - otherwise no new
//  communications
//  * can start.
//  *
...
// 	// shortcut activation, but it requires cascade tag (CT) byte, that is
// not
// 	// part of uid.
...
//  * Only 4 bytes can be specified in a SELECT command, so for the longer
//  UIDs two
//  * or three iterations are used: UID size	Number of UID bytes
//  Cascade
...
// 					  ///< Normally 0. If set you must also supply
// uid->size. ) { 	bool uidComplete; 	bool selectDone; 	bool
// useCascadeTag; 	uint8_t cascadeLevel = 1; 	int result; 	uint8_t
// count; 	uint8_t checkBit; 	uint8_t index; 	uint8_t uidIndex; // The
// first index in uid->uidByte[] that is used in the
...
// 	uint8_t bufferUsed; // The number of bytes used in the buffer, ie the
// number
// 						// of bytes to transfer to the FIFO.
// 	uint8_t rxAlign; // Used in BitFramingReg. Defines the bit position for
// the
// 					 // first bit received.
...
// See
// 	//explanation below. CT means Cascade Tag. 		Byte 3: UID-data
// Byte 4: UID-data
// 	//		Byte 5: UID-data
// 	//		Byte 6: BCC					Block Check Character - XOR of bytes
// 2-5
// 	//		Byte 7: CRC_A
// 	//		Byte 8: CRC_A
// 	// The BCC and CRC_A are only transmitted if we know all the UID bits of
// the
// 	// current Cascade Level.
// 	//
// 	// Description of bytes 2-5: (Section 6.5.4 of the ISO/IEC 14443-3
// draft:
...
// 		collReg, 0x80); // ValuesAfterColl=1 => Bits received after
// collision
// 						// are cleared.
...
// 			(currentLevelKnownBits % 8
// 					? 1
// 					: 0); // The number of bytes needed to represent the
// known
// 						  // bits for this level.
// 		if (bytesToCopy) {
// 			uint8_t maxBytes =
// 				useCascadeTag ? 3 : 4; // Max 4 bytes in each Cascade Level.
// 									   // Only 3 left if we use the Cascade
// Tag 			if (bytesToCopy > maxBytes) { 				bytesToCopy =
// maxBytes;
// 			}
...
// 				32) { // All UID bits in this Cascade Level are known. This
// is a
// 					  // SELECT.
...
// 				bufferUsed = 9;
// 				// Store response in the last 3 bytes of buffer (BCC and
// CRC_A -
// 				// not needed after tx)
...
// 				index = 2 + count; // Number of whole bytes: SEL + NVB +
// UIDs 				buffer[1] = 					(index << 4) +
// txLastBits; // NVB - Number of Valid Bits 				bufferUsed =
// index + (txLastBits ? 1 : 0);
...
// 				}
// 				if (collisionPos <=
// 					currentLevelKnownBits) { // No progress - should not
// happen 					return STATUS_INTERNAL_ERROR;
// 				}
...
// Level 					currentLevelKnownBits = 32;
...
// buffer[] 		bytesToCopy = (buffer[2] == PICC_CMD_CT) ? 3 : 4;
// for (count = 0; count < bytesToCopy; count++) {
...
// 		if (responseLength != 3 ||
// 			txLastBits != 0) { // SAK must be exactly 24 bits (1 byte +
// CRC_A). 			return STATUS_ERROR;
// 		}
// 		// Verify CRC_A - do our own calculation and store the control in
