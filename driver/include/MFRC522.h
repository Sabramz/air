 ...
 *
 * @authors Dr.Leong, Miguel Balboa, Søren Thing Andersen, Tom Clement, many
 * more! See GitLog.
 *
...
#define __STDC_LIMIT_MACROS
	...
	#define MFRC522_SPICLOCK                                                   \
		(4000000u) // MFRC522 accept upto 10MHz, set to 4MHz.
#endif
...
// 2005; 16.1 self-test
/* const byte MFRC522_firmware_referenceV0_0[] = {0x00, 0x87, 0x98, 0x0f,
	0x49, 0xFF, 0x07, 0x19, 0xBF, 0x22, 0x30, 0x49, 0x59, 0x63, 0xAD, 0xCA,
	0x7F, 0xE3, 0x4E, 0x03, 0x5C, 0x4E, 0x49, 0x50, 0x47, 0x9A, 0x37, 0x61,
	0xE7, 0xE2, 0xC6, 0x2E, 0x75, 0x5A, 0xED, 0x04, 0x3D, 0x02, 0x4B, 0x78,
	0x32, 0xFF, 0x58, 0x3B, 0x7C, 0xE9, 0x00, 0x94, 0xB4, 0x4A, 0x59, 0x5B,
	0xFD, 0xC9, 0x29, 0xDF, 0x35, 0x96, 0x98, 0x9E, 0x4F, 0x30, 0x32, 0x8D};
// Version 1.0 (0x91)
// NXP Semiconductors; Rev. 3.8 - 17 September 2014; 16.1.1 self-test
const byte MFRC522_firmware_referenceV1_0[] = {0x00, 0xC6, 0x37, 0xD5,
	0x32, 0xB7, 0x57, 0x5C, 0xC2, 0xD8, 0x7C, 0x4D, 0xD9, 0x70, 0xC7, 0x73,
	0x10, 0xE6, 0xD2, 0xAA, 0x5E, 0xA1, 0x3E, 0x5A, 0x14, 0xAF, 0x30, 0x61,
	0xC9, 0x70, 0xDB, 0x2E, 0x64, 0x22, 0x72, 0xB5, 0xBD, 0x65, 0xF4, 0xEC,
	0x22, 0xBC, 0xD3, 0x72, 0x35, 0xCD, 0xAA, 0x41, 0x1F, 0xA7, 0xF3, 0x53,
	0x14, 0xDE, 0x7E, 0x02, 0xD9, 0x0F, 0xB5, 0x5E, 0x25, 0x1D, 0x29, 0x79};
// Version 2.0 (0x92)
// NXP Semiconductors; Rev. 3.8 - 17 September 2014; 16.1.1 self-test
const byte MFRC522_firmware_referenceV2_0[] PROGMEM = {0x00, 0xEB, 0x66, 0xBA,
	0x57, 0xBF, 0x23, 0x95, 0xD0, 0xE3, 0x0D, 0x3D, 0x27, 0x89, 0x5C, 0xDE,
	0x9D, 0x3B, 0xA7, 0x00, 0x21, 0x5B, 0x89, 0x82, 0x51, 0x3A, 0xEB, 0x02,
	0x0C, 0xA5, 0x00, 0x49, 0x7C, 0x84, 0x4D, 0xB3, 0xCC, 0xD2, 0x1B, 0x81,
	0x5D, 0x48, 0x76, 0xD5, 0x71, 0x61, 0x21, 0xA9, 0x86, 0x96, 0x83, 0x38,
	0xCF, 0x9D, 0x5B, 0x6D, 0xDC, 0x15, 0xBA, 0x3E, 0x7D, 0x95, 0x3B, 0x2F}; */
...
public:
	...
	enum PCD_Register : byte {
		...
		WaterLevelReg = 0x0B
						<< 1,   // level for FIFO underflow and overflow warning
		...
		TxControlReg = 0x14 << 1, // controls the logical behavior of the
		...
		TPrescalerReg = 0x2B << 1, // the lower 8 bits of the TPrescaler value.
		...
		TestPinValueReg = 0x34 << 1, // defines the values for D1 to D7 when it
									 // is used as an I/O bus
	...
	enum PICC_Command : byte {
		...
		PICC_CMD_REQA = 0x26, // REQuest command, Type A. Invites PICCs in state
							  // IDLE to go to READY and prepare for
		...
		// PCD_MFAuthent to authenticate access to a sector, then use these
		// commands to read/write/modify the blocks on the sector. The
		// read/write commands can also be used for MIFARE Ultralight.
		...
		PICC_CMD_MF_WRITE =
			0xA0, // Writes one 16 byte block to the authenticated sector of the
				  // PICC. Called "COMPATIBILITY WRITE" for MIFARE Ultralight.
		PICC_CMD_MF_DECREMENT =
			0xC0, // Decrements the contents of a block and stores the result in
				  // the internal data register.
		PICC_CMD_MF_INCREMENT =
			0xC1, // Increments the contents of a block and stores the result in
				  // the internal data register.
		PICC_CMD_MF_RESTORE = 0xC2,  // Reads the contents of a block into the
									 // internal data register.
		PICC_CMD_MF_TRANSFER = 0xB0, // Writes the contents of the internal data
									 // register to a block.
	...
	// add more. last value set to 0xff, then compiler uses less ram, it seems
	// some optimisations are triggered
	enum PICC_Type : byte {
		PICC_TYPE_UNKNOWN,
	...
	// GetStatusCodeName() if you add more. last value set to 0xff, then
	// compiler uses less ram, it seems some optimisations are triggered
	...
	MFRC522(const gpiod::chip &chip,
		uint32_t inter_pin,
		byte chipSelectPin, ///< Arduino pin connected to MFRC522's SPI slave
							///< select input (Pin 24, NSS, active low)
		byte resetPowerDownPin, ///< Arduino pin connected to MFRC522's reset
								///< and power down input (Pin 6, NRSTPD, active
								///< low). If there is no connection from the
								///< CPU to NRSTPD, set this to UINT8_MAX. In
								///< this case, only soft reset will be used in
								///< PCD_Init().
		std::string adapter);
	...
	void PCD_ReadRegister(
		PCD_Register reg, byte count, byte *values, byte rxAlign = 0);
	...
	/////////////////////////////////////////////////////////////////////////////////////
	StatusCode PCD_TransceiveData(byte *sendData,
		byte sendLen,
		byte *backData,
		byte *backLen,
		...
		byte rxAlign = 0,
		bool checkCRC = false);
	StatusCode PCD_CommunicateWithPICC(byte command,
		byte waitIRq,
		byte *sendData,
		byte sendLen,
	...
	StatusCode PICC_REQA_or_WUPA(
		byte command, byte *bufferATQA, byte *bufferSize);
	...
	StatusCode PCD_Authenticate(
		byte command, byte blockAddr, MIFARE_Key *key, Uid *uid);
	void PCD_StopCrypto1();
	StatusCode MIFARE_Read(byte blockAddr, byte *buffer, byte *bufferSize);
	StatusCode MIFARE_Write(byte blockAddr, byte *buffer, byte bufferSize);
	StatusCode MIFARE_Ultralight_Write(
		byte page, byte *buffer, byte bufferSize);
	...
	StatusCode PCD_MIFARE_Transceive(
		byte *sendData, byte sendLen, bool acceptTimeout = false);
	...
	void PICC_DumpMifareClassicToSerial(
		Uid *uid, PICC_Type piccType, MIFARE_Key *key);
	void PICC_DumpMifareClassicSectorToSerial(
		Uid *uid, MIFARE_Key *key, byte sector);
	void PICC_DumpMifareUltralightToSerial();

	// Advanced functions for MIFARE
	void MIFARE_SetAccessBits(
		byte *accessBitBuffer, byte g0, byte g1, byte g2, byte g3);
